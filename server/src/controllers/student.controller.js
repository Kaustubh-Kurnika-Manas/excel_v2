...
module.exports = {
    ...
    getProfile: async (req, res, next) => {
        try {
            if (req.user.mentoredBy && req.user.mentoredBy.length > 0) {
                const mentors = await Mentor.find({ _id: { $in: req.user.mentoredBy } });
                req.user.mentoredBy = mentors;
            }
            // Ensure we have all required fields
            const profileData = {
                ...req.user.toObject(),
                firstname: req.user.firstname || req.user.menteeName || req.user.name || "",
    ...
    getStudentsByMentor: async (req, res, next) => {
        try {
            let students = await Student.find({ mentoredBy: { $in: [req.user._id] } })
                .select("-password -tokens");
            response.success(res, "", students);
            next();
        } catch (err) {
